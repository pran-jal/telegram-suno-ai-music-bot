...
class suno_client:
    ...
    def generate_songs(self, telegram_prompt):
        ...
        for clip in generated_response.get("clips"):
    ...
    def process_generated_songs(self, generate_response):
        clips = generate_response.get("clips")[0]
        ...
        try:
            ...
            while True:
                ...
                for song_id, audio_url in self.song_ids.items():
                    if not audio_url:
                        self.song_ids[song_id] = self.process_generated_songs(self.check_if_generated(song_id))
                        break
                else:
                    return list(self.song_ids.values())        
